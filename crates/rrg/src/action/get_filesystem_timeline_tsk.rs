...
impl FromLossy<crate::fs::Entry> for rrg_proto::get_filesystem_timeline_tsk::Entry {
...
mod tests {
    ...
    fn handle_file_attributes() {
        ...
        // Windows API.
        let mut temp_path_wstr = temp_path
            .as_path()
            .as_os_str()
            .encode_wide()
            .collect::<Vec<_>>();
        ...
        let status = unsafe { SetFileAttributesW(temp_path_wstr.as_ptr(), FILE_ATTRIBUTE_HIDDEN) };
        assert!(status > 0);
    ...
    fn path(entry: &rrg_proto::get_filesystem_timeline::Entry) -> Option<PathBuf> {
